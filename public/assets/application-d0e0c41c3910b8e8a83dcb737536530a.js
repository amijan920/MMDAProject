    ...
    selection.each(function() {
      ...
      function createAxis(dom) {
        ...
        if (typeof axis === "boolean") {
          ...
          axis = d3.svg.axis()
              .scale(d3.scale.linear().domain([0,24]).range([0, sliderLength]))
              .ticks(24)
...
BubbleChart = (function() {
	function BubbleChart(data, overall_data) {
		...
		this.addDataSet("overall", overall_data, 0);
	...
	BubbleChart.prototype.switchDataSet = function(filterName) {
		this.active_filter = filterName;
		this.switchData()
	}

	BubbleChart.prototype.switchTime = function(time) {
		this.active_time = time;
		this.refreshData();
	} 

	BubbleChart.prototype.addDataSet = function(filterName, data, time) {

		console.log(this.station_start);
		for(var i = 0; i < data.length; i++) {
			var north = [];
			var south = [];
			var ave = [];
...
function createChart() {
	...
	root.switchTime = (function(_this) {
		return function(time) {
			return chart.switchTime(time);
		};
	})(this);

	root.addDataSet = (function(_this) {
		return function(filterName, data, time) {
			return chart.addDataSet(filterName, data, time);
		}
	})
	...
	return d3.csv("assets/line_stations.csv", function(data1) {
		return d3.text("assets/averaged_data/weekday.txt", function(text) {
...
TimeSlider = (function() {
	...
	TimeSlider.prototype.switchData = function(data) {
